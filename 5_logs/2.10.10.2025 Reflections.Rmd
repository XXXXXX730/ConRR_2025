...
neutr <- read.csv('1_data','neutrophils.csv') # first trial of data importing, I did this approach as I confused it with a previoulsy used approach of importing data into R project with the funtions `rio::import()` and `here()`.

neutro <- read.csv('1_data/neutrophils.csv') # corrected way importing when we want to futher organize our project as puting every inmput, script and output could also get messy.
...
neutro.2 <- rio::import(here('1_data','neutrophils.csv')) # advantageous as it can import data of any sort with the same code and no code adaptation is needed, header identification, and works with R script and R markdown .. not needed reference handbook of epidemiologists
